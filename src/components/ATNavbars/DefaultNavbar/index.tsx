...
import DefaultNavbarDropdown from "components/ATNavbars/DefaultNavbar/DefaultNavbarDropdown";
import DefaultNavbarMobile from "components/ATNavbars/DefaultNavbar/DefaultNavbarMobile";
    ...
    if (collapse && columns && name === dropdownName) {
      ...
      template = collapse.map((item: any) => {
        ...
        return (
          ...
          >
            {item.description ? (
              <ATBox display="flex" py={0.25} fontSize="1rem" color="text">
                {typeof item.icon === "string" ? (
                  <Icon color="inherit">{item.icon}</Icon>
                ) : (
                  <ATBox color="inherit">{item.icon}</ATBox>
                )}
                <ATBox pl={1} lineHeight={0}>
                  <ATTypography
                    variant="button"
                    display="block"
                    fontWeight="bold"
  ...
  const dropdownMenu = (
    ...
    >
      {({ TransitionProps }) => (
        ...
        >
          <ATBox borderRadius="lg">
            <ATTypography variant="h1" color="white">
              <Icon ref={setArrowRef} sx={{ mt: -3 }}>
                arrow_drop_up
              </Icon>
            </ATTypography>
            <ATBox shadow="lg" borderRadius="lg" p={1.625} mt={1}>
              {renderRoutes}
            </ATBox>
          </ATBox>
        </NewGrow>
  ...
  const nestedDropdownMenu = (
    ...
    >
      {({ TransitionProps }) => (
        ...
        >
          <ATBox ml={2.5} mt={-2.5} borderRadius="lg">
            <ATBox shadow="lg" borderRadius="lg" py={1.5} px={1} mt={2}>
              {renderNestedRoutes}
            </ATBox>
          </ATBox>
        </NewGrow>
    ...
    <Container>
      ...
      >
        <ATBox
          ...
          pl={{ xs: 0, lg: 1 }}
        >
          <ATTypography variant="button" fontWeight="bold" color={light ? "white" : "dark"}>
            {brand}
          </ATTypography>
        </ATBox>
        <ATBox color="inherit" display={{ xs: "none", lg: "flex" }} m={0} p={0}>
          {renderNavbarItems}
        </ATBox>
        {action &&
          (action.type === "internal" ? (
            <ATBox display={{ xs: "none", lg: "inline-block" }}>
              <ATButton
                component={Link}
                to={action.route}
                variant="gradient"

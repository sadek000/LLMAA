...
const i18n = ["entries_per_page", "showing", "to", "of", "entries", "search"];
...
function DataTable({
  ...
  const [entriesPerPageText, showing, to, ofText, entriesText, searchText] = useATTranslation(i18n);
    ...
    <TableContainer sx={{ boxShadow: "none" }}>
      {entriesPerPage || canSearch ? (
        <ATBox display="flex" justifyContent="space-between" alignItems="center" p={3}>
          ...
          {entriesPerPage && (
            <ATBox display="flex" alignItems="center" ml="auto">
              <Autocomplete
                disableClearable
        ...
        {showTotalEntries && (
          <ATBox mb={{ xs: 3, sm: 0 }}>
            <ATTypography variant="button" color="secondary" fontWeight="regular">
              {showing} {entriesStart} {to} {entriesEnd} {ofText} {rows.length} {entriesText}
            </ATTypography>
          </ATBox>
        )}
